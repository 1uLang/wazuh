...
void test_syscollector_dbsync_valid_msg_unknown_operation(void **state)
{
    ...
    expect_string(__wrap__mdebug1, formatted_msg, "Unable to send dbsync information to Hids DB.");
    int ret = DecodeSyscollector(lf, 0);
...
void test_syscollector_dbsync_valid_msg_query_error(void **state)
{
    ...
    expect_string(__wrap__merror, formatted_msg, "Hids-db query error, check wdb logs.");
    expect_string(__wrap__mdebug1, formatted_msg, "Unable to send dbsync information to Hids DB.");
