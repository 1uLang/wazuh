...
int DecodeSyscollector(Eventinfo *lf,int *socket)
{
    ...
    if (strcmp(msg_type, "port") == 0 || strcmp(msg_type, "port_end") == 0) {
        if (decode_port(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send ports information to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_package(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send packages information to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hotfix(lf, logJSON, socket) < 0) {
            mdebug1("Unable to send hotfixes information to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hardware(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send hardware information to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_osinfo(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send osinfo message to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_netinfo(lf, logJSON, socket) < 0) {
            merror("Unable to send netinfo message to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_process(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send processes information to Hids DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_dbsync(lf->agent_id, msg_type, logJSON, socket) < 0) {
            mdebug1("Unable to send %s information to Hids DB.", msg_type);
            cJSON_Delete (logJSON);
            return (0);
        }
    }
            ...
            if (NULL != operation_object && NULL != data && cJSON_IsString(operation_object) && cJSON_IsObject(data)) {
                ...
                if (NULL != field_list) {
                    ...
                    if (fill_data_dbsync(data, field_list, msg) && TRUE == msg->status) {
                        ...
                        if (ret_val != 0) {
                            merror("Hids-db query error, check wdb logs.");
                        }
                    } else {
