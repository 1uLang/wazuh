...
function install
{
    ...
    cmd /c start (Get-Item ".\hids-agent*.msi").Name -quiet -norestart -log installer.log
}

# Check new version and restart the hids service
function check-installation
{
    ...
    write-output "$(Get-Date -format u) - Restarting Hids service." >> .\upgrade\upgrade.log
    Get-Service -Name "Hids" | Start-Service
}
...
# Get process name
$current_process = "hids-agent"
If (!(Test-Path ".\hids-agent.exe"))
{
...
Get-Service -Name "Hids" | Stop-Service -ErrorAction SilentlyContinue -Force
$process_id = (Get-Process $current_process -ErrorAction SilentlyContinue).id
$counter = 5
while($process_id -ne $null -And $counter -gt 0)
{
    write-output "$(Get-Date -format u) - Trying to stop Hids service again. Remaining attempts: $counter." >> .\upgrade\upgrade.log
    $counter--
    Get-Service -Name "Hids" | Stop-Service
...
$process_id = (Get-Process hids-agent).id
$counter = 5
while($process_id -eq $null -And $counter -gt 0)
{
    $counter--
    Start-Service -Name "Hids"
    Start-Sleep 2
    $process_id = (Get-Process hids-agent).id
}
...
Start-Sleep 10
# Check status file
$status = Get-Content .\hids-agent.state | select-string "status='connected'" -SimpleMatch
$counter = 5
