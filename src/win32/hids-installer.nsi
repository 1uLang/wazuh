...
!define REVISION "40219"
!define NAME "Hids"
!define SERVICE "HidsSvc"

; output file
!ifndef OutFile
    !define OutFile "hids-agent-${VERSION}.exe"
!endif

Var is_upgrade

Name "${NAME} Windows Agent v${VERSION}"
BrandingText "Copyright (C) 2015-2020, Hids Inc."
OutFile "${OutFile}"

VIProductVersion "4.0.0.0"
VIAddVersionKey ProductName "${NAME}"
VIAddVersionKey CompanyName "Hids Inc."
VIAddVersionKey LegalCopyright "2020 - Hids Inc."
VIAddVersionKey FileDescription "Hids Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "Hids Agent"
...
Section "Hids Agent (required)" MainSec
    ...
    ; install files
    File hids-agent.exe
    File hids-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-hids.exe restart-hids.exe
    ...
    File /oname=libhidsext.dll ..\libhidsext.dll
    File /oname=libhidsshared.dll ..\libhidsshared.dll
    ...
    ; use appropriate version of "hids-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\hids-agent.exe"
        Rename "$INSTDIR\hids-agent-eventchannel.exe" "$INSTDIR\hids-agent.exe"
    ${Else}
        Delete "$INSTDIR\hids-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "Hids, Inc."
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://www.zhiannet.com"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://www.zhiannet.com"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\hids-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\hids-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
