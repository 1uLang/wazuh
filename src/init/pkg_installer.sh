...
while [ "$status" != "connected" -a $COUNTER -gt 0 ]; do
    . ${WAZUH_HOME}/var/run/hids-agentd.state >>${WAZUH_HOME}/logs/upgrade.log 2>&1
    sleep 1
    COUNTER=$[COUNTER - 1]
...
if [ "$status" = "connected" -a $RESULT -eq 0 ]; then
    ...
    CONTROL="$WAZUH_HOME/bin/hids-control"
    if [ ! -f $CONTROL ]; then
    ...
    if command -v semodule >/dev/null && command -v getenforce >/dev/null; then
        if [ $(getenforce) != "Disabled" ]; then
            if [ -f ${WAZUH_HOME}/var/selinux/hids.pp ]; then
                echo "$(date +"%Y/%m/%d %H:%M:%S") - Restoring SELinux policy ...." >>${WAZUH_HOME}/logs/upgrade.log
                semodule -i ${WAZUH_HOME}/var/selinux/hids.pp >>${WAZUH_HOME}/logs/upgrade.log 2>&1
                semodule -e hids >>${WAZUH_HOME}/logs/upgrade.log 2>&1
            else
                echo "$(date +"%Y/%m/%d %H:%M:%S") - ERROR: Hids SELinux module not found." >>${WAZUH_HOME}/logs/upgrade.log
            fi
        else
            echo "$(date +"%Y/%m/%d %H:%M:%S") - Hids SELinux module installation is skipped (SELinux is disabled)." >>${WAZUH_HOME}/logs/upgrade.log
        fi
    fi
