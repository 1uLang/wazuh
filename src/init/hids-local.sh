...
AUTHOR="Hids Inc."
DAEMONS="hids-modulesd hids-monitord hids-logcollector hids-syscheckd hids-analysisd hids-maild hids-execd wazuh-db hids-agentlessd hids-integratord wazuh-dbd hids-csyslogd"
...
start_service()
{
    ...
    TEST=$(${DIR}/bin/hids-logtest-legacy -t  2>&1)
    echo $TEST

    if [ ! -z "$TEST" ]; then
        echo "hids-analysisd: Configuration error. Exiting."
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/hids-control start
    fi
...
stop_service()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/hids-control stop
    fi
