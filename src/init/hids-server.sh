...
AUTHOR="Hids Inc."
USE_JSON=false
DAEMONS="hids-clusterd hids-modulesd hids-monitord hids-logcollector hids-remoted hids-syscheckd hids-analysisd hids-maild hids-execd wazuh-db hids-authd hids-agentlessd hids-integratord wazuh-dbd hids-csyslogd hids-apid"
OP_DAEMONS="hids-clusterd hids-maild hids-agentlessd hids-integratord wazuh-dbd hids-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Hids $VERSION..."
    fi

    TEST=$(${DIR}/bin/hids-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/hids-analysisd.failed
        exit 1;
    fi
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Hids, removing..."
                fi
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/hids-execd//')
    lock
    stop_service
    start_service
    unlock
