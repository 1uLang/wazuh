...
def test_WazuhDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    ...
    query_group._data = [{'count': 1, 'name': 'hids-master'},
                         {'count': 1, 'name': 'hids-agent1'}]
...
def test_WazuhDBQueryGroupByAgents_format_data_into_dictionary_status(mock_socket_conn):
    ...
    query_group._data = [
        {'os.name': 'Ubuntu', 'count': 1, 'version': 'Hids v4.0.0', 'status': 'disconnected',
         'lastKeepAlive': 1593093968},
        {'os.name': 'Ubuntu', 'count': 2, 'version': 'Hids v3.13.0', 'status': 'disconnected',
         'lastKeepAlive': 1593093968},
        {'os.name': 'Ubuntu', 'count': 1, 'version': 'Hids v3.13.0', 'status': 'disconnected',
...
def test_agent_add(mock_add_authd, mock_add_manual, authd_status, ip, id, key, force):
    ...
    with patch('wazuh.core.agent.get_manager_status', return_value={'hids-authd': authd_status}):
...
@patch('wazuh.core.agent.get_manager_status', return_value={'hids-authd': 'stopped'})
...
def test_agent_getconfig(socket_mock, send_mock, mock_wazuh_socket):
    ...
    result = agent.getconfig('com', 'active-response', 'Hids v4.0.0')
...
@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Hids v4.2.0', 'Hids v4.0.0', 'Hids v4.2.1', 'Hids v3.13.2']])
])
@patch('wazuh.core.agent.WazuhQueue')
