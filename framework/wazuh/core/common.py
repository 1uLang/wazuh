...
def call_wazuh_control(option) -> str:
    wazuh_control = os.path.join(find_wazuh_path(), "bin", "hids-control")
    try:
...
def get_wazuh_version() -> str:
    return get_wazuh_info("HIDS_VERSION")


@lru_cache(maxsize=None)
def get_wazuh_revision() -> str:
    return get_wazuh_info("HIDS_REVISION")


@lru_cache(maxsize=None)
def get_wazuh_type() -> str:
    return get_wazuh_info("HIDS_TYPE")


wazuh_path = find_wazuh_path()

install_type = metadata['install_type']
wazuh_version = metadata['hids_version']
...
analysisd_stats = os.path.join(wazuh_path, 'var', 'run', 'hids-analysisd.state')
remoted_stats = os.path.join(wazuh_path, 'var', 'run', 'hids-remoted.state')
...
AR_LEGACY_VERSION = 'Hids v4.2.0'
ACTIVE_CONFIG_VERSION = 'Hids v3.7.0'

# Command variables
CHECK_CONFIG_COMMAND = 'check-manager-configuration'
RESTART_WAZUH_COMMAND = 'restart-hids'
